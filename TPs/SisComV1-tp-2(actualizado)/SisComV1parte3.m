...
%Parámetros
DataLength = 10000;         % Cantidad de símbolos a transmitir con 1M se tilda
...
N0 = 1/EsNo(k);
N01=0.126;
ecualizador = EcMMSE(canal,Tabs,N0); % se generan los tabs del ecualizador MMSE
                                % EcZF(canal,Tabs) --> si uso ZF (no es el caso)
                                % EcMMSE(x,taps,N0) --> si uso el otro
                                % x=canal
Gx=(conv(canal, [1 0])); % se pretende generar una respuesta global con un solo simbolo sobremuestreado para poder calcular el retardo inicial
Hx=(conv(Gx, ecualizador));
Retardo = round(length(Hx)/2);

if (mod(Retardo,2)==0) % se decide a partir de donde comenzar a mostrar las muestras antes del primer simbolo, solo para mejores efectos visuales, pero debe elegirse adecuadamente para no errarle al submuestreo
    Inicio = 2;
else
    Inicio = 1;
end
...
RxPAMSignal = conv(NoisySignal,ecualizador); %señal ecualizada sobremuestreada
    Recu = RxPAMSignal(Inicio:end); % se recorta la señal ecualizada en el inicio. de esta manera se puede apreciar la respuesta al impulso (solo cuando se envia un solo bit)
    Recu = downsample(Recu,2); %señal ecualizada submuestreada
    Detector = Recu(((Retardo-Inicio+1)/2):end); % se comienzan a detectar los simbolos
    Constelacion = Recu(((Retardo-Inicio+1)/2):DataLength+((Retardo-Inicio+1)/2)-1); % se toman las muestras sin detectar submuestreadas para graficar el diagrama de constelaciones
