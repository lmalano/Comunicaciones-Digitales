%PUNTO 2: USAR CANAL DEL EJEMPLO 6.11 DE PROAKIS. PROBAR SOLO Y CON
%ECUALIZADOR
...
ecualizador = EcZF(canal,Tabs);
% ecualizador = [1];

Gx=(conv(canal, [1 0])); % canal upsampleado
Hx=(conv(Gx, ecualizador)); %Hx no se usa para el ejercicio pero sirve para mostrar la respuesta al impulso ecualizada
...
%graficos de señales ecualizada y sin ecualizar
   ...
   title('diagrama de constelacones de señal ecualizada')
    ...
    stem(Hx)
    title('rta impulsiva ecualizada del canal')
