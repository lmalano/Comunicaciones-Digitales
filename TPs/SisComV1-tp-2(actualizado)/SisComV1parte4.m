...
%Parámetros
DataLength = 100;         % Cantidad de símbolos a transmitir
...
No = 0.03;                            % OFF: Apaga el canal de ruido gaussiano
EsNodB = 1;               % Determina la relación en decibeles entre la energia del símbolo Es y la densidad espectral de potencia de ruido No
EsNo = 1/No;
...
ecu=EcLMS(NoisySignal,1000,K,DataLength,sigma,SymbolsAux)
...
r = Retardo - (Tabs-1)/2;
     ...
     Recu = downsample(Hx,2); %señal ecualizada submuestreada
    Detector = Hx(((Retardo-Inicio+1)/2):end);
