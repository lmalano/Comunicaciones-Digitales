...
TxPAMSignal = conv(Symbols,RCFilter);       % Conformación de la señal a transmitir
% figure(4)
% stem(TxPAMSignal)
% title('señal enviada')
...
ecualizador = ecualizadorMMSE(SRRCFilter,1/EsNo(k));
RxPAMSignal = conv(ecualizador,NoisySignal); % señal ecualizada
% RxPAMSignal =downsample(RxPAMSignal,10)
% figure(1)
% stem(RxPAMSignal)
% title('señal ecualizada')
elseif strcmp(RxFilter,'NotMatched')
    RxPAMSignal = conv(NoisySignal,ones(1,51));
end        
% if length(EsNodB)==1
% figure(4)
% stem(RxPAMSignal);grid;
% end
RetardoIni = 76; %126;      %Retardo para tener en cuenta los transitorios producidos por las conv al inicio y al final de la transmisión
...
disp('error teorico')
erTeorico = 0.5*erfc(sqrt(EsNo)) %consultar por que no es sqrt(2*Esno)
figure
